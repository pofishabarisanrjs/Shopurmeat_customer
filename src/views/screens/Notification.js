...
const readMark=(item,data)=>{
  ...
  .then((itemstatus) => {
    ...
    console.log('Mendal', itemstatus[0].orderstatus.name)
    setStatus(itemstatus[0].orderstatus.name)
  })
  .catch((error) => console.error(error))
  ...
  body: JSON.stringify({
   ...
   //   console.log("sabaris ---->",result)
      ...
      const unreadDataLength = unreadData.length;
setNotificationnumber(unreadDataLength)      
      if(data.unique_order_id && status !== "Canceled" && status !=="Delivered" )
      {
        navigation.navigate('OrderDetailscreen', {
          id: item.id,
          orderid: data.unique_order_id,
...
useFocusEffect(
  React.useCallback(() => {
    ...
    if(userInfo?.auth_token !==undefined)
    Notificationapi()
  }, [])
);
